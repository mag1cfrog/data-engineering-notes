...
tags:
  ...
  - optimization
  - window-functions
description: "How a simple ROW_NUMBER() window function caused 391GB of disk spilling and 33+ minute execution times"
draft: true
---
  ...
  SELECT 
    ...
    ROW_NUMBER() OVER (
      ...
      ORDER BY 
        ...
        processed_day DESC, 
        tie_breakers (x, y, z coordinates)
    ) AS rn
...
#### The Double Shuffle-Sort Mystery
...
#### Understanding What Happened: Sort-based Aggregation
...
#### The Double Penalty: Why We Had Multiple Sorts
...
- **Disk I/O bottleneck**: Once spilling started (391GB!), the entire pipeline became I/O bound
...
#### The Real Problem: We Don't Need Full Sorting!
...
We were essentially sorting millions of rows in each partition just to pick the "top 1"—that's massively inefficient!

#### The Solution: max_by Function
...
so it seems like as long as we don't use a different collation than uft8_binary then all the strings should be binary stable.
